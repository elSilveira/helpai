...
def _build_xai_request_body(wav_bytes: bytes) -> tuple[bytes, str]:
    ...
    language = (XAI_STT_LANGUAGE or "").strip()
    if language:
        if XAI_STT_FORMAT_TEXT:
            write_text_field("format", "true")
        write_text_field("language", language)
...
def _transcribe_with_xai(wav_bytes: bytes) -> str:
    if not XAI_API_KEY:
        raise RuntimeError(
    ...
    body, boundary = _build_xai_request_body(wav_bytes)
...
def transcribe_audio_array(audio: np.ndarray, sample_rate: int = 16000, provider: str | None = None) -> str:
    ...
    if active_provider == "local":
        return transcribe_local(normalized_audio, sample_rate)

    try:
        return _transcribe_with_xai(_audio_array_to_wav_bytes(normalized_audio, sample_rate))
    except Exception:
...
def transcribe_wav_bytes(wav_bytes: bytes, provider: str | None = None) -> str:
    ...
    if active_provider == "xai":
        try:
            return _transcribe_with_xai(wav_bytes)
        except Exception:
            if not _use_local_fallback_on_error(provider):
                raise
            logger.exception("xAI STT failed; falling back to local faster-whisper.")
